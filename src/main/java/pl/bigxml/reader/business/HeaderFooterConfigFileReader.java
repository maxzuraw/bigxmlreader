...
public class HeaderFooterConfigFileReader {
    ...
    public List<HeaderFooterConfig> read() throws Exception {
        List<HeaderFooterConfig> result = new ArrayList<>();
        ...
        try (InputStreamReader inputStreamReader = new InputStreamReader(resourceFile.getInputStream()); CSVReader reader = new CSVReaderBuilder(inputStreamReader).withCSVParser(parser).build()) {
            ...
            while ((line = reader.readNext()) != null) {
                HeaderFooterConfig headerFooterConfig = HeaderFooterConfig.builder()
                        .xmlPath(line[0])
